...
my %hash_mrna = (); # hash to count how many mrna per gene to identify alternative splicing

my $pattern = "\^.*ID=\(\\w+\)\(:\\w+\)?\(:\\d+\)?;.+\$"; # pattern to extract the gene id
		...
		unless(/^#/){  # To avoid lines which start with '#'
			unless(/^\s*$/){  # This is to avoid lines with no relevant content
				...
				if(lc($feature) eq "gene"){
					$hash_genes{$scaffold}{$gene_id}{GENE_START} = $cols[POS_START];
					$hash_genes{$scaffold}{$gene_id}{GENE_END} = $cols[POS_END];
					$hash_genes{$scaffold}{$gene_id}{GENE_STRAND} = $cols[STRAND];
					$hash_genes{$scaffold}{$gene_id}{NOTE} = $hash_notes{$gene_id};
				}
				...
				if(exists $hash_genes{$scaffold}{$gene_id}{FEATURES}{$feature}){
					my $array_genes = $hash_genes{$scaffold}{$gene_id}{FEATURES}{$feature};
					...
					#if($hash_genes{$scaffold}{$gene_id}{GENE_START} < $hash_line->{POS_START}){
					#	$hash_genes{$scaffold}{$gene_id}{GENE_START} = $hash_line->{POS_START};
					#}
					## we consider the gene end position as the major position of all features
					#if($hash_genes{$scaffold}{$gene_id}{GENE_END} > $hash_line->{POS_END}){
					#	$hash_genes{$scaffold}{$gene_id}{GENE_END} = $hash_line->{POS_END};
					#}
					...
					$hash_genes{$scaffold}{$gene_id}{FEATURES}{$feature} = \@array_genes;
					#$hash_genes{$scaffold}{$gene_id}{GENE_START} = $hash_line->{POS_START};
					#$hash_genes{$scaffold}{$gene_id}{GENE_END} = $hash_line->{POS_END};
					#$hash_genes{$scaffold}{$gene_id}{GENE_STRAND} = $hash_line->{STRAND};
					#$hash_genes{$scaffold}{$gene_id}{NOTE} = $hash_notes{$gene_id};
				}
			}
		}
...
sub sort_cds{
	for(@array_scaffolds_order){
		...
		for(@$array_genes_order){
			my $gene_id = $_;
			my @features = sort keys %{$hash_genes{$scaffold}{$gene_id}{FEATURES}};

			if($hash_genes{$scaffold}{$gene_id}{GENE_STRAND} eq "+"){
				for(@features){
					my @array_ordered = sort {$a->{POS_START} <=> $b->{POS_START} || $a->{POS_END} <=> $b->{POS_END} }  @{$hash_genes{$scaffold}{$gene_id}{FEATURES}{$_}};
					$hash_genes{$scaffold}{$gene_id}{FEATURES}{$_} = \@array_ordered;
				}
			}else{
				for(@features){
					my @array_ordered = sort {$b->{POS_START} <=> $a->{POS_START} || $b->{POS_END} <=> $a->{POS_END} }  @{$hash_genes{$scaffold}{$gene_id}{FEATURES}{$_}};
					$hash_genes{$scaffold}{$gene_id}{FEATURES}{$_} = \@array_ordered;
				}
			}
		}
...
sub generate_tbl{
	for(@array_scaffolds_order){
		...
		for(@$array_genes_order){
			my $gene_id = $_;
			if($hash_genes{$scaffold}{$gene_id}{GENE_STRAND} eq "+"){
				print OUT sprintf("%d\t%d\tgene\n", $hash_genes{$scaffold}{$gene_id}{GENE_START}, $hash_genes{$scaffold}{$gene_id}{GENE_END});
			}else{
				print OUT sprintf("%d\t%d\tgene\n", $hash_genes{$scaffold}{$gene_id}{GENE_END}, $hash_genes{$scaffold}{$gene_id}{GENE_START});
			}
			...
			for(@features){
				my $feature = $_;
				if($_ ne "gene"){
					for(my $i = 0; $i < scalar @{$hash_genes{$scaffold}{$gene_id}{FEATURES}{$feature}}; $i++){
						my $feature_info = $hash_genes{$scaffold}{$gene_id}{FEATURES}{$feature}->[$i];
						if($i and $feature ne "exon"){
							if($feature_info->{STRAND} eq "+"){
								print OUT sprintf("%d\t%d\n", $feature_info->{POS_START}, $feature_info->{POS_END});
							}else{
								print OUT sprintf("%d\t%d\n", $feature_info->{POS_END}, $feature_info->{POS_START});
							}
						}else{
							if($feature_info->{STRAND} eq "+"){
								print OUT sprintf("%d\t%d\t%s\n", $feature_info->{POS_START}, $feature_info->{POS_END}, $feature);
							}else{
								print OUT sprintf("%d\t%d\t%s\n", $feature_info->{POS_END}, $feature_info->{POS_START}, $feature);
							}

							if($feature eq "exon"){
								print OUT sprintf("\t\t\tnumber %d\n", ($i+1));
							}
							if(lc($feature) eq "trna"){
								print OUT sprintf("\t\t\tproduct\ttRNA-Phe\n");
							}

						}
					}
					if(uc($feature) eq "CDS" or uc($feature) eq "MRNA"){
						print OUT sprintf("\t\t\tproduct\t%s\n", $hash_genes{$scaffold}{$gene_id}{NOTE});
						print OUT sprintf("\t\t\tprotein_id\tgnl|PcoaUGA|%s\n", $gene_id);
						print OUT sprintf("\t\t\ttranscript_id\tgnl|PcoaUGA|mrna.%s\n", $gene_id);
					}
				}
			}
		}
...
sub get_hash_from_line{
	...
	$hash{POS_START} = trim($cols[POS_START]);
	$hash{POS_END} = trim($cols[POS_END]);
	$hash{STRAND} = trim($cols[STRAND]);
...
#########################################################################
# Just a report to see strange things at the gff [not tested after the second version]
#########################################################################
sub print_report{
